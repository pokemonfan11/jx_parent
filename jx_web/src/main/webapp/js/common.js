...
function formatDateTimeCN(date) {
	...
	if (date.length == 4) { // yyyy
		...
		s = date.replace("-0", "-").replace("-", "年").replace("-", "月")
				.replace(" 0", " ").replace(" ", "日")
				+ "时";
	} else if (date.length == 16) { // yyyy-MM-dd HH:mm
		s = date.replace("-0", "-").replace("-", "年").replace("-", "月")
				.replace(" 0", " ").replace(" ", "日").replace(":0", ":")
				.replace(":", "时")
				+ "分";
...
function iteratorFrames(framesArray) {
	for (var i = 0; i < framesArray.length; i++) {
		...
		// alert("1 --" + childWindowFrames.name + " : " +
		// childWindowFrames.isCannotSubmit);
		if (childWindowFrames.isCannotSubmit) {
			// alert(childWindowFrames.isCannotSubmit);
			// alert(windowFrames[0].isCanSubmit);
			childWindowFrames.isCannotSubmit = 0;
			// alert(windowFrames[0].isCanSubmit);
			// alert("2 --" + childWindowFrames.name + " : " +
			// childWindowFrames.isCanSubmit);
...
function popupWindow(url, windowObject) {
	...
	var returnValues = showModalDialog(url, windowObject, 'dialogWidth:'
			+ dialogBoxWidth + 'px;dialogHeight:' + dialogBoxHeight
			+ 'px;dialogLeft:' + dialogBoxLeft + 'px;dialogTop:' + dialogBoxTop
			+ 'px;center:yes;help:no;resizable:yes;status:no');
	if (returnValues) {
		return returnValues;
	}
	// window.open(url, "请选择", "top=" + dialogBoxTop + ",left=" + dialogBoxLeft
	// + ",width=" + dialogBoxWidth + ",height=" + dialogBoxHeight +
	// ",alwaryRaised=yes,dependent=yes,location=no,menubar=no,resizable=yes,scrollbars=yes,toolbar=no");
}
...
function popupWindowModeless(url, windowObject) {
	...
	window.showModelessDialog(url, windowObject, 'dialogWidth:'
			+ dialogBoxWidth + 'px;dialogHeight:' + dialogBoxHeight
			+ 'px;dialogLeft:' + dialogBoxLeft + 'px;dialogTop:' + dialogBoxTop
			+ 'px;center:yes;help:no;resizable:yes;status:no');
	// window.open(url, "请选择", "top=" + dialogBoxTop + ",left=" + dialogBoxLeft
...
function listTableColor(tableId) {
	...
	for (var i = 0; i < oTable.tBodies[0].rows.length; i++) {
		if (oTable.tBodies[0].rows[i].className != 'errorMessage') {
			oTable.tBodies[0].rows[i].className = (i % 2 == 0) ? 'oddTr'
					: 'evenTr';
		}
	}
}
...
/*
 * 复制table最后一个tr的内容 并创建新tr objId:table 的id属性
 */
...
/*
 * 删除table复选框指定的tr 主要结合 addRecord使用 tableId:table 的 id 属性
 * topRow:table顶端有几行不参与删除操作 clearValueObjName: 在移除的时候哪一个值需要清空。
...
/*
 * 2006/10/12 @xiongtao 功能：超链接高亮显示． 用法：<a id="aa_3"
 * onclick="linkHighlighted(this)"> id 命名必需以 aa_开头，后边填写数字， 超链接id名称不能重复。
 * 
 */
...
/*
 * 2006/10/20 @oasis 功能：MainMenu超链接高亮显示． 用法：<a id="aa_3"
 * onclick="linkHighlighted(this)"> id 命名必需以 aa_开头，后边填写数字， 超链接id名称不能重复。
 * 
 */
...
/*
 * 设置隐藏层 objMain: 设置隐藏层ID； PS: test1 objss : 设置未隐藏后DIV ID； PS: test_visibility1
 * 比如点击后为 - objother: 设置隐藏前DIV ID； test_own1 比如点击前为 +
 * PS:ShowFloatDiv('testObj',1,5) obj,第几个,个数； 2008.11.05 by Vicco
 */
function ShowFloatDiv(obj, num, length) {
	for (var id = 1; id <= length; id++) {
		...
		if (id == num && objMain) {
			try {
				document.getElementById(objMain).style.display = "block";
			} catch (e) {
			}
			;
			try {
				document.getElementById(objss).style.display = "none";
			} catch (e) {
			}
			;
			try {
				document.getElementById(objother).style.display = "block";
			} catch (e) {
			}
			;
		}
...
function HideFloatDiv(obj, num, length) {
	for (var id = 1; id <= length; id++) {
		...
		if (id == num) {
			...
			;
			try {
				document.getElementById(objss).style.display = "block";
			} catch (e) {
			}
			;
			try {
				document.getElementById(objother).style.display = "none";
			} catch (e) {
			}
			;
		}
...
function clearcon(obj, length) {
	...
	if (!Maxnum) {
		document.getElementById(message).innerHTML = " <font color=#009900>"
				+ "0" + "&nbsp;" + "/" + "&nbsp;" + "无上限" + "</font>";
	} else {
		document.getElementById(message).innerHTML = "<font color=#009900>"
				+ "0" + "&nbsp;" + "/" + "&nbsp;" + Maxnum + "</font>";
...
/*
 * add by AAA 2011-04-27 清空当前text框内容
 */
...
/*
 * update by AAA 2011-04-26 检验页面文本框等内容长度，汉字占两个字符
 */
function getMaxlength(id) {
	...
	if (txtobj != null) {
		...
		var shownum = 'num_' + id;
		if (!Maxnum) {
			document.getElementById(shownum).innerHTML = " <font color=#009900>"
					+ intLength + "&nbsp;" + "/" + "&nbsp;" + "无上限" + "</font>";
...
/*
 * 功能：跟linkHighlighted类似，用于恢复页面初始超链接高亮显示． 用法：<a
 * onclick="resumeLinkHighlighted()"> 需恢复的超链A标签的 id 命名必需以 aa_开头，超链接id名称不能重复。
 * 
 */
...
/*
 * 2006/10/16 功能：取消当前页面所有dTree高亮显示和当前选中项． 用法：<a onclick="unSelectedAlldTree()">
 * 
 */
...
/*
 * 2006/10/23 @laukin 功能：url重定向。 用法：<a onclick="urlDispatch('index.html')">首页</a>
 */
...
/*
 * 2006/10/23 @laukin 功能：判断表单元素是否修改，改变不同的样式。
 * 
 */
...
function checkCompareDateArrays(big_Name, small_Name) {
	...
	if (big_DateArray && small_DateArray) {
		...
		for (var i = 0; i < big_DateArray.length; i++) {
			...
			if (isTrue) {
				alertMessage += "第" + (i + 1) + "条【"
						+ big_DateArray[i].dispName + "】不能小于【"
						+ small_DateArray[i].dispName + "】</br>";
			}
		}
	}
	if (alertMessage) {
...
/**
 * 用来判断何种状态下，可以进行操作！ serviceName:操作名； selectObjId:复选框id； canNotStatus：不可操作的状态；
		...
		for (i = 0; i < objectArray.length; i++) {
			if (objectArray[i].checked) {
				...
				if (statusHidden == canNotStatus) {
					// alert(statusHidden);
					alertString += "第" + (i + 1) + "条记录，归档状态不符合进行【"
							+ serviceName + "】操作的条件，请核查！<br>";
				}
			}
		}
	}
...
function clearThis() {
	var obj = window.event.srcElement;
	var rul = /^(\d{4})\/(\d{2})\/(\d{2})$/; // 如果更改 格式"0000/00/00"
												// 则需要更改此正则表达式
...
/**
 * ---isCanSign---- ---Oscar---2007-08-02--------- 判断是否可以进行签订合同操作
 * serviceName:操作名； selectObjId:单选框id； canNotStatus：不可操作的状态；
 * 注：使用此方法必须在页面selectObjId复选框增加自定义属性：currentStatus。存放当前状态。
 * 
 */
	...
	if (alertString) {
		...
		for (i = 0; i < objectArray.length; i++) {
			if (objectArray[i].checked) {
				statusHidden = objectArray[i].currentStatus;
				if (statusHidden == canNotStatus) {
					alertString += "第" + (i + 1) + "条记录，合同签订状态不符合进行【"
							+ serviceName + "】操作的条件，请核查！<br>";
				}
			}
		}
...
/**
 * ---isArchLocle---- ---Oscar---2007-08-08--------- 判断是否可以进行签订合同操作
 * serviceName:操作名； selectObjId:单选框id； canNotStatus：不可操作的状态；
	...
	if (alertString) {
		...
		for (i = 0; i < objectArray.length; i++) {
			if (objectArray[i].checked) {
				...
				if (statusHidden == canNotStatus) {
					alertString += "第" + (i + 1) + "条记录，档案未确定(未归档)不符合进行【"
							+ serviceName + "】操作的条件，请核查！<br>";
				}
			}
		}
...
/**
 * ---isCanStop---- ---Oscar---2007-09-24--------- 判断是否可以进行停用操作 serviceName:操作名；
 * selectObjId:单选框id； canNotStatus：不可操作的状态；
 */
	...
	if (alertString) {
		...
		for (i = 0; i < objectArray.length; i++) {
			if (objectArray[i].checked) {
				statusHidden = objectArray[i].currentStatus;
				if (statusHidden == canNotStatus) {
					alertString += "第" + (i + 1) + "条记录，人员信息状态不符合进行【"
							+ serviceName + "】操作的条件，请核查！<br>";
				}
			}
		}
...
/**
 * -----ifCanOpt----- ------LiuFan-------------- 判断是否可以进行[修改、删除]操作 serviceName:
 * 操作名； selectObjId: 单选框id； stateAble： 可以操作的状态；
 */
		...
		for (i = 0; i < objectArray.length; i++) {
			if (objectArray[i].checked) {
				...
				if (currentStatus != stateAble) {
					errorMessage = "此条工作已上报,不能进行【" + serviceName
							+ "】操作!!!<br/>";
				}
			}
		}
...
/*
 * create by tony 2008-11-15 para: objName=属性所属对象; propertyName=属性名;
 * stateAble=可通过的值; msg=出错信息;
 */
		...
		for (i = 0; i < objectArray.length; i++) {
			if (objectArray[i].checked) {
				...
				if (statusHidden == canStatus) {
					alertString += "第" + (i + 1) + "条工作任务是由计划生成，不能进行【"
							+ serviceName + "】操作！<br>";
				}
			}
		}
...
function sizeChange(name, size) {
	...
	if (parseInt(obj.style.height.replace("px", "")) < 200) {
...
/*
 * 页面加载后检查是否已加入个人关注列表，如果有则添加对应样式，并取消当前鼠标事件以及A标签； 如果未增加到关注列表，则添加对应鼠标事件；
 * 给指定ID的TD添加鼠标事件; 2008.11.27 By Vicco
 */
function CheckAtts() {
	var DarkTR = document.getElementsByTagName('td');
	for (i = 0; i < DarkTR.length; i++) {
		if (attentionArray
				.indexOf(DarkTR[i].parentNode.parentNode.parentNode.id) != -1) {
			...
			DarkTR[i].innerHTML = TEXT
					+ '<img style=\"margin-left:10px;\" src=\"../skin/default/images/notice.gif\">'; /* 自动为已加入关注的内容后增加对应文字或图示 */
			// alert('YES');
		}
	}
	...
	if ((attentionArray.push('${empAtt}') - 1) == 0) {
		...
		imptt.innerHTML = '<a href=\"empAttentionListAction.do\" title=\"点击查看详细列表\" >'
				+ '<img style=\"position:relative;top:-1px;margin-right:8px;\" src=\"../skin/default/images/notice.gif\">'
				+ "我已有"
				+ "<span>"
				+ "<font color=#990000>"
				+ (attentionArray.push('${empAtt}') - 2)
				+ "</font>"
				+ "</span>" + "条关注的工作" + "</a>";
	}
...
function CustomMouse(obj) {
	...
	for (i = 0; i < DardTD.length; i++) {
		if (attentionArray
				.indexOf(DardTD[i].parentNode.parentNode.parentNode.id) != -1
				|| attentionArray.indexOf(DardTD[i].parentNode.id) != -1) {
...
/*
 * 定义鼠标点击事件 无需判断任何条件，只做点击后的事件处理； 点击后转由CheckAtts()函数处理点击后的样式；
 */
...
function ChangePosition(obj, length) {
	...
	if (obj.firstChild.className == '3A'
			&& obj.firstChild.innerText.length > length) {
...
function interceptTextarea(obj, Width) {
	...
	if (!obj.previousSibling) {
		...
		obj
				.insertAdjacentHTML(
						"beforeBegin",
						'<iframe class="iframeC" style="position:absolute;z-index:9997;width:expression(this.nextSibling.offsetWidth);height:expression(this.nextSibling.offsetHeight);top:expression(this.nextSibling.offsetTop);left:expression(this.nextSibling.offsetLeft);filter:progid:DXImageTransform.Microsoft.Alpha(opacity=0);"></iframe>');
	} // 在该对象前加入IFRAME，如果已含有则略过,加入IFRAME的目的是为了遮盖SELECT选择框
	// obj.onkeyup = function(){this.scrollHeight > 10 ? this.style.height =
	// this.scrollHeight + 'px' : ''}; //输入时如果滚动高度大于10则自动扩大到有效高度
...
/*
 * 根据鼠标点击位置确定弹出窗口位置并防止溢出屏幕 function positionXY(elmId,h){ var divID =
 * document.getElementsByTagName('div'); for(i=0;i<divID.length;i++){
 * if(divID[i].id.indexOf(elmId) != -1){ obj = window.event.srcElement; objTop =
 * obj.offsetTop; objParent = obj.offsetParent;
 * while(objParent.tagName.toUpperCase() != "BODY"){ objTop +=
 * objParent.offsetTop; objParent = objParent.offsetParent; } var ss
 * =document.body.clientHeight - objTop; if(ss < h){ divID[i].style.top = objTop -
 * h + 'px'; //h
 * 为鼠标点击位置距离页面底边的高；这里为如果鼠标点击位置局里底边的高小于该层（h）的高度，则该层的位置为鼠标点击位置减去次高度，即向上方弹出； }else{
 * divID[i].style.top = objTop + 20 +'px'; //20 为鼠标点击处向下定位40像素 } //alert(h); } }
 * 
 * //alert(o.style.top); }
 */
...
/*
 * 设置普通输入框鼠标样式事件，排除带有特殊图标的输入框，如日期等。排除选择框等INPUT 2008.11.25 by Vicco
 */
function reInput() {
	var inputs = document.getElementsByTagName('input');
	for (i = 0; i < inputs.length; i++) {
		if (inputs[i].type != "checkbox" && inputs[i].type != "radio"
				&& inputs[i].className != "hand_date"
				&& inputs[i].className != "hand"
				&& inputs[i].className != "hand_choose"
				&& inputs[i].type != "button" && inputs[i].dataType != "复选框"
				&& inputs[i].name != "addDept_txt" && inputs[i].id != "percent") {
			inputs[i].onfocus = function() {
				this.style.border = "1px solid #208073";
				this.style.background = "#edfaf9"
			}; // 激活事件边框与背景
			inputs[i].onblur = function() {
				this.style.background = "#fff"
			}; // 非激活背景
			inputs[i].onmouseover = function() {
				this.style.border = "1px solid #e2994a"
			}; // 鼠标事件
			inputs[i].onmouseout = function() {
				this.style.border = "1px solid #369e92"
			};
		} else if (inputs[i].className == "hand_date"
				|| inputs[i].className == "hand_choose") {
	...
	for (i = 0; i < textArea.length; i++) {
		if (textArea[i].id.indexOf("textarea") != -1) {
			...
			textArea[i].onmouseout = function() {
				this.style.border = "1px solid #369e92"
			};
		}
	}
