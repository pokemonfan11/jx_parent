...
import cn.ajajaaj.utils.file.PoiUtil;
...
public class ContractPrint{
	...
	/**
	 * 排序的思路
	 * 1 抽离出所有的工厂名字，不重复:光华、宏艺、海洋、杞县海洋
	 * 2 遍历名字：找到每个工厂的货物，放在一起，光华、光华、光华、宏艺、宏艺、海洋、海洋、杞县海洋、杞县海洋、杞县海洋、、
	 * 
	 * 
	 * 
	 */
	public void print(Contract contract,String path, HttpServletResponse response) throws Exception{
		...
		List<ContractProduct> oList1 = new ArrayList<ContractProduct>();
		//将oSet放入oList中
		oList1.addAll(oSet);
		//排序
		Set<String> factoryNames = new HashSet<String>();
		//货物有10个，但是此处的factoryNames的长度<=10 :光华、宏艺、海洋、杞县海洋、
		for(ContractProduct cp:oList1){
			factoryNames.add(cp.getFactoryName());
		}
		//集合
		...
		for(String factoryName:factoryNames){
			...
			for(int i =0;i<oList1.size();i++){
				ContractProduct cp = oList1.get(i);
				if(cp.getFactoryName().equals(factoryName)){
					oList.add(cp);
//					oList1.remove(cp);
				}
			}
		...
		//填写每页的内容，之后在循环每页读取打印
		Map<String,String> pageMap = null;//封装的每页的数据
		...
		for(int i=0;i<oList.size();i++){
			...
			pageMap.put("CheckBy", "审单："+ utilFuns.fixSpaceStr(UtilFuns.convertNull(contract.getCheckBy()),26)+"验货员："+utilFuns.convertNull(contract.getInspector()));
...
//		sheet.removeRowBreak(6);
		...
		for(int p=0;p<pageList.size();p++){
			...
			if(UtilFuns.isNotEmpty(printMap.get("Cnumber")) && UtilFuns.isNotEmpty(printMap.get("Price"))){
				//单元格必须要设置为CELL_TYPE_FORMULA格式，才允许写公司
			...
			nCell = nRow.createCell(1);
			nCell.setCellValue(printMap.get("Crequest").replaceAll("\\\\r\\\\n", "\r\n"));
